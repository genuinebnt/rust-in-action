...
fn calculate_mandlebrot(
    ...
    for img_y in 0..height {
        ...
        for img_x in 0..width {
            let cx = x_min + (x_max - y_min) * (img_x as f64 / width as f64);
            let cy = y_min + (y_max - x_min) * (img_y as f64 / height as f64);
            let escaped_at = mandlebrot_point(cx, cy, max_iters);
            row.push(escaped_at);
        }
        all_rows.push(row);
    }
...
fn mandlebrot_point(cx: f64, cy: f64, max_iters: usize) -> usize {
